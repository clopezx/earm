...
# MBax + MBax <-->  Bax2
simpleadd(Bax(state='M'), Bax2(bf=None), kbaxdimf, kbaxdimr)
...
# Bax2 + Bax2 <-->  Bax4
simpleadd(Bax2(), Bax4(bf = None), kbaxtetf, kbaxtetr)
...
twostepmod(MitoP(state='A'), CytoC(state='mito'), CytoC(bf = None, state='A'),
           kmitopcytocMf, kmitopcytocMr, kmitopcytocMc)

# AMito + mSmac <-->  AMito:mSmac --> AMito + ASmac  
twostepmod(MitoP(state='A'), Smac(state='mito'), Smac(bf = None, state='A'),
...
twostepmod(CytoC(state='cyto'), Apaf(state='I'), Apaf(bf = None, state = 'A'),
           kcytocCapaff, kcytocCapafr, kcytocCapafc)

# aApaf + pC9 <-->  Apop
simpleadd(Apaf(state='A', C9(), Apop(bf=None),
                kapafc9f, kapafc9r)

# Apop + pC3 <-->  Apop:pC3 --> Apop + C3
twostepmod(Apop(), C3(state='pro'), C3(bf = None, state='A'),
