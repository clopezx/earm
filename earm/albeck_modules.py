...
def pore_to_parp_DR():
    """ This is the same module as that in the original Albeck paper
    but the rules that involve Caspase 8 have been modified to work
    seamlessly with the development version of the DISC module
    """
   ...
   # --------------------------------------
    ...
    # replace original catalyze function with dimer
    # catalyze(C8(state='A'), C3(state='pro'), C3(state='A'), [1e-7, KR, KC])
    catalyze(C8dim, C3(state='pro'), C3(state='A'), [1e-7, KR, KC])
    catalyze(XIAP(), C3(state='A'), C3(state = 'ub'), [2e-6, KR, 1e-1])
    catalyze(C3(state='A'), PARP(state='U'), PARP(state='C'), [KF, 1e-2, KC])
    catalyze(C3(state='A'), C6(state='pro'), C6(state='A'), [KF, KR, KC])
    catalyze(C6(state='A'), C8(state='pro'), C8(state='A'), [3e-8, KR, KC])
    catalyze(C8(state='A'), C8(state='A'), C8dim(bf=None), [KF, KR, KC])
